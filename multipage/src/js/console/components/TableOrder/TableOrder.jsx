...
export default class extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        if (prevState.orderInfo_list !== this.state.orderInfo_list) {
            let row = this.state.orderInfo_list[0]
            if (this.state.orderInfo_list.length > 0)
                this.handleOnSelect(row,true)
                // this.setState(() => ({
                //     selected: [this.state.orderInfo_list[0].orderId]
                // }));
    ...
    Test1 = (e) => {
        ...
        this.setState({ selected: ['202010090008'] })
    ...
    handleOnSelect = (row, isSelect) => {
        this.setState({selected: [row.orderId]})
